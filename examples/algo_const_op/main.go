...
func main() {
	...
	fmt.Printf("Count 2: %v\n", algorithm.Count[int](a.Begin(), a.End(), 2))
	fmt.Printf("Count 2: %v\n", algorithm.CountIf[int](a.Begin(), a.End(), isEven))

	iter := algorithm.Find[int](a.Begin(), a.End(), 2)
	if !iter.Equal(a.End()) {
		fmt.Printf("Fund %v\n", iter.Value())
	}
	...
	iter = algorithm.MaxElement[int](a.Begin(), a.End())
	if !iter.Equal(a.End()) {
		fmt.Printf("largest value : %v\n", iter.Value())
	}
	iter = algorithm.MinElement[int](a.Begin(), a.End())
	if !iter.Equal(a.End()) {
		fmt.Printf("largest value : %v\n", iter.Value())
	}
}
